...
### "Cannot connect to API" — k6'da

```bash
# API gerçekten healthy mi?
docker compose ps

# Eğer "starting" görüyorsan, healthcheck henüz geçmemiş — birkaç saniye bekle.
docker compose ps --format json | jq '.[] | {name: .Name, state: .State, health: .Health}'
```

### Grafana boş gösteriyor

1. Prometheus targets'ı kontrol et: http://localhost:9090/targets
2. `threading-api` job'u UP olmalı
3. Eğer DOWN ise: API container'a Prometheus erişemiyor demektir, network'ü kontrol et

### Test sonuçları tutarsız

- Her testten önce 60 saniye bekle (ThreadPool reset)
- Docker Desktop'a yeterli kaynak ver
- Background process'leri kapat (browser, IDE)
- Birden fazla test üst üste çalıştırma — kuyruğa al

## Sonraki Adım: GCP

Bu setup'ta her şey çalışıyorsa GCP'ye taşımak basit:

1. `docker-compose.yml` aynen Cloud Run veya GCE'de kullanılabilir
2. Sadece API ve k6 GCP'ye gider; Prometheus + Grafana lokal tutulabilir (cloud-managed alternatifleri de var)
3. Network değişikliği: API'nin public IP veya internal load balancer'a bağlanması

## Yapı Özeti

```
threading-bench-local/
├── docker-compose.yml           ← 4 servis: api, k6, prometheus, grafana
├── src/
│   ├── Dockerfile               ← Multi-arch (ARM64 + AMD64)
│   ├── Program.cs               ← Minimal API + OpenTelemetry
│   └── ThreadingDemo.csproj
├── k6/
│   ├── stress-test.js           ← Ramping yük
│   └── constant-load.js         ← Sabit yük
├── prometheus/
│   └── prometheus.yml           ← 5sn scrape interval
└── grafana/
    ├── dashboards/
    │   └── threading-dashboard.json   ← 7 panel, hazır dashboard
    └── provisioning/
